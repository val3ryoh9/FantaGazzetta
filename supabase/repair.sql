...
alter table public.league_data add column if not exists coppa_teams jsonb not null default '[]'::jsonb;
alter table public.league_data add column if not exists coppa_matches jsonb not null default '[]'::jsonb;
...
drop policy if exists "authenticated users can list active leagues" on public.leagues;
...
drop policy if exists "members can view their memberships" on public.league_members;
create policy "members can view their memberships"
on public.league_members for select to authenticated
using (user_id = auth.uid());

drop policy if exists "members can read their league data" on public.league_data;
...
drop policy if exists "admins can write their league data" on public.league_data;
...
create or replace function public.enter_league_existing(target_league_id uuid)
returns table (league_id uuid, role text)
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if not coalesce(v_is_admin, false) and v_role is null then
    raise exception 'Utente non autorizzato per questa lega';
  end if;

  return query select target_league_id as league_id,
    case when coalesce(v_is_admin, false) then 'admin' else v_role end as role;
end;
$$;

grant execute on function public.enter_league_existing(uuid) to authenticated;

-- Mantieni attive esclusivamente le due leghe ufficiali.
update public.leagues
set name = 'FantaCirco',
    password_hash = extensions.crypt('DonniniLeague', extensions.gen_salt('bf')),
    active = true
where name = 'Fantacampo 2024'
  and not exists (select 1 from public.leagues where name = 'FantaCirco');

update public.leagues
set password_hash = extensions.crypt('DonniniLeague', extensions.gen_salt('bf')),
    active = true
where name = 'FantaCirco';
...
update public.leagues
set active = false
where name not in ('FantaCirco', 'Federazione Italiana Pippe');
